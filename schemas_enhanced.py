...
class PomodoroSessionBase(BaseModel):
    session_type: str = Field(..., regex="^(work|short_break|long_break)$")
...
class AIEnhanceRequest(BaseModel):
    text: str = Field(..., min_length=1)
    mode: str = Field(default="improve", regex="^(improve|summarize|expand|translate|restructure)$")
    model: Optional[str] = None
...
class UserSettingsBase(BaseModel):
    ...
    language: str = Field(default="zh", regex="^(zh|en)$")
